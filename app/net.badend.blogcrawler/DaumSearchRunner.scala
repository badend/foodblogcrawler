...
object DaumSearchRunner {
  ...
  def daumProcessSearch() = {
    ...
    val daumURLS = Files.newBufferedWriter(Paths.get(s"${System.getProperty("user.dir")}/data/daum/daum_search_URLS.${DateTime.now.toIsoDateString}"), Charset.forName("UTF8"))
    var cnt = 0
    while (true) {
